...
(defun children (node) (cdddr node))
(defun nodename (node) (car node))
(defun nodeweight (node) (cadr node))
(defun constructnode (nodename nodeweight &rest children)
  (apply #'append (list nodename) (list nodeweight) (when (not (eql children nil)) `((->) ,@children)))) 
(defun find-node (nodename) (loop for i in *nodelist* when (equalp nodename (nodename i)) return i))

(defun fill-children (node)
  (loop for i in (children node)
       collect (find-node i)))

(defun constructwithchildren (node)
  (constructnode (nodename node) (nodeweight node)
		 (fill-children node)))
       
(defun nodelistiter (nodelist)
  (loop for node in nodelist
   when (children node)
     collect (constructwithchildren node)))

(defun findinchildren (node nodelist)
  (loop for i in nodelist when
       (loop for j in (children i)
	  when (equalp j node) return i)
     return i))

(defun childrenweight (node)
  (loop for i in (children node)
       for j = (find-node i)
     collect (cons (nodeweight j) (childrenweight j))))

(defun childrenweight2 (node)
  (loop for i in (children node)
       for j = (find-node i)
     summing (+ (nodeweight j) (childrenweight2 j))))

(defun balanceweight (node)
  (loop for i in (children node)
     for j = (find-node i)
     for k = (+ (nodeweight j) (childrenweight2 j))
     collect (cons i k )))

;;; * (balanceweight (find-node 'wiapj))
;;; 
;;; ((DJZJIWD . 88556) (LSIRE . 88563) (VLBIVGC . 88556) (XDCTKBJ . 88556)
;;;  (YGVPK . 88556))
;;; * (balanceweight (find-node 'ycpcv))
;;; 
;;; ((LOCRTXL . 8957) (SHLFZ . 8957) (YCPCV . 8964))
;;; * (balanceweight (find-node 'eionkb))
;;; 
